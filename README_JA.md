...
> 🐳 **クイック試用:** `edict/docker-compose.yml` のDocker Composeを使い、http://127.0.0.1:7899 を開いてください。
...
cd edict/edict
cp .env.example .env
# AstrBot/OpenClaw連携を使う場合は .env を編集してください。
docker compose up -d --build
```

開く：

- ダッシュボード: http://127.0.0.1:7899
- バックエンドヘルスチェック: http://127.0.0.1:7898/health

> 旧 `docker run -p 7891:7891 ...`、`dashboard/server.py`、`run_loop.sh` の起動手順は廃止済みです。
...
このインストーラーは旧OpenClaw workspace初期化用にのみ残しています。通常のデプロイは上記Docker Composeを使ってください。
...
├── install.sh                  # 旧OpenClaw workspace初期化スクリプト
├── start.sh                    # Docker Compose起動ヘルパー
├── edict.service               # Docker Compose用systemdラッパー
├── edict.sh                    # Docker Composeサービス管理ヘルパー
└── LICENSE                     # MIT
```
...
| **FastAPIバックエンド** | SQLAlchemy、Alembic、Redisを使う非同期APIサービス |
| **Docker Composeスタック** | `edict/docker-compose.yml` からフロントエンド、バックエンド、Postgres、Redis、Workerを起動 |
