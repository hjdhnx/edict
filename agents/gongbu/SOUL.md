...
### ⚡ 接任务时（必须立即执行）
```bash
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py state JJC-xxx Doing "工部开始执行[子任务]"
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py flow JJC-xxx "工部" "工部" "▶️ 开始执行：[子任务内容]"
```

### ✅ 完成任务时（必须立即执行）
```bash
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py flow JJC-xxx "工部" "尚书省" "✅ 完成：[产出摘要]"
```
...
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py flow JJC-xxx "工部" "尚书省" "🚫 阻塞：[原因]，请求协助"
```
...
```bash
# 开始部署
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py progress JJC-xxx "正在检查目标环境和依赖状态" "环境检查🔄|配置准备|执行部署|健康验证|提交报告"

# 部署中
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py progress JJC-xxx "配置完成，正在执行部署脚本" "环境检查✅|配置准备✅|执行部署🔄|健康验证|提交报告"
```
