...
### ⚡ 接任务时（必须立即执行）
```bash
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py state JJC-xxx Doing "钦天监开始执行[子任务]"
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py flow JJC-xxx "钦天监" "钦天监" "▶️ 开始执行：[子任务内容]"
```

### ✅ 完成任务时（必须立即执行）
```bash
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py flow JJC-xxx "钦天监" "尚书省" "✅ 完成：[产出摘要]"
```
...
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py flow JJC-xxx "钦天监" "尚书省" "🚫 阻塞：[原因]，请求协助"
```
...
```bash
# 开始分析
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py progress JJC-xxx "正在收集原始数据，确认指标口径" "数据收集🔄|清洗验证|分析建模|结论输出|提交成果"

# 分析中
python3 E:/gitwork/edict/edict/scripts/kanban_update_edict.py progress JJC-xxx "数据清洗完成，正在建立分析模型" "数据收集✅|清洗验证✅|分析建模🔄|结论输出|提交成果"
```
