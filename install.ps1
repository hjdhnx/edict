...
Write-Host ""
Write-Host "下一步："
Write-Host "  1. 配置当前主栈环境变量:"
Write-Host "     cd edict; copy .env.example .env"
Write-Host "     # 按需设置 DISPATCH_BACKEND、ASTRBOT_API_URL、ASTRBOT_API_KEY"
Write-Host "  2. 启动当前 Docker Compose 主栈:"
Write-Host "  3. 启动服务:          docker compose up -d --build"
Write-Host "  4. 打开总控台:        http://127.0.0.1:7899"
Write-Host ""
Warn "本脚本仅保留 OpenClaw 旧集成初始化；当前常规部署请优先使用 Docker Compose + AstrBot。"
Info "文档: README.md / docs/getting-started.md"
