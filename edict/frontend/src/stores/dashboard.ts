...
export type TabKey = 'edicts' | 'court' | 'monitor' | 'officials' | 'models' | 'skills' | 'sessions' | 'memorials' | 'templates' | 'morning' | 'about';
...
export function getPipeStatus(t: Task) {
...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const officialsData = ref<OfficialsData | null>(null);
  const agentsStatusData = ref<AgentsStatusData | null>(null);
  const morningBrief = ref<MorningBrief | null>(null);
  const subConfig = ref<SubConfig | null>(null);

  const activeTab = ref<TabKey>('edicts');
  const edictFilter = ref<'active' | 'archived' | 'all'>('active');
  const sessFilter = ref('all');
  const tplCatFilter = ref('全部');
  const selectedOfficial = ref<string | null>(null);
  const modalTaskId = ref<string | null>(null);
  const countdown = ref(5);
  const toasts = ref<{ id: number; msg: string; type: 'ok' | 'err' }[]>([]);

  const tasks = computed(() => liveStatus.value?.tasks || []);
  const edicts = computed(() => tasks.value.filter(isEdict));
  const activeEdicts = computed(() => edicts.value.filter((t) => !isArchived(t)));
  ...
  async function loadLive() {
    try {
      ...
      if (!officialsData.value) {
        api.officialsStats().then((data) => (officialsData.value = data)).catch((error) => logger.debug({ error }, 'preload officials failed'));
      }
    } catch (error) {
      logger.warn({ error }, 'load live status failed');
    }
  }
  ...
  async function loadAgentConfig() {
    try {
      ...
      logger.warn({ error }, 'load agent config failed');
    }
  }

  async function loadOfficials() {
    try {
      officialsData.value = await api.officialsStats();
    } catch (error) {
      logger.warn({ error }, 'load officials failed');
    }
  }

  async function loadAgentsStatus() {
    try {
      agentsStatusData.value = await api.agentsStatus();
    } catch (error) {
      agentsStatusData.value = null;
      logger.warn({ error }, 'load agents status failed');
    }
  }
  ...
  async function loadMorning() {
    try {
      ...
      morningBrief.value = brief;
      subConfig.value = config;
    } catch (error) {
      logger.warn({ error }, 'load morning failed');
    }
  }

  async function loadSubConfig() {
    try {
      subConfig.value = await api.morningConfig();
    } catch (error) {
      logger.warn({ error }, 'load subscription config failed');
    }
  }

  async function loadAll() {
    await loadLive();
    if (['models', 'skills'].includes(activeTab.value)) await loadAgentConfig();
    if (activeTab.value === 'monitor') await loadAgentsStatus();
    if (activeTab.value === 'officials') await loadOfficials();
    if (activeTab.value === 'morning') await loadMorning();
  }

  function setActiveTab(tab: TabKey) {
    activeTab.value = tab;
    logger.info({ tab }, 'switch tab');
    if (['models', 'skills', 'sessions'].includes(tab) && !agentConfig.value) loadAgentConfig();
    if (tab === 'officials' && !officialsData.value) loadOfficials();
    if (tab === 'monitor') loadAgentsStatus();
    if (tab === 'morning' && !morningBrief.value) loadMorning();
  }
  ...
  function startRealtime() {
    ...
    try {
      ...
      logger.warn({ error }, 'websocket create failed');
      startPolling();
      return;
    }
    ws.onopen = () => {
      logger.info('websocket connected');
      ws?.send(JSON.stringify({ type: 'subscribe', topics: ['task.*'] }));
    };
    ws.onmessage = (event) => {
      try {
        ...
        logger.debug({ error }, 'ignore malformed websocket frame');
      }
    };
    ws.onclose = () => {
